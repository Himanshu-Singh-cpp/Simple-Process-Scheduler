...
void history()
{
    ...
    for (int i = 0; i < num_commands; i++)
    {
        ...
        printf("CPU burst time %f\n",total_time-history_arr[i]->time);
                    ...
                    if (i < count_current)
                    {
                        if (kill(temp_arr[i]->pid, SIGSTOP) != -1)
                        {
                            ...
                            if (wpid == 0)
                            {
                                ...
                                if (temp_arr[i]->cycles>4  && ((temp_arr[i]->cycles)%5==0)){
                                    temp_arr[i]->priority=1;
                                }
                                else if (temp_arr[i]->priority < 4)
                                {
